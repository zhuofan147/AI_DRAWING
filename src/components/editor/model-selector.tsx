...
export function InlineModelPicker({ capability, value: controlledValue, onChange }: InlineModelPickerProps) {
  ...
  const options = useMemo(() => {
    ...
    // Fallback: if no models for this capability, include models from all providers
    // (same API key often works for image+video, user may have miscategorized)
    if (result.length === 0) {
      for (const p of providers) {
        if (!p || !p.models) continue;
        for (const m of p.models) {
          ...
          result.push({
            providerId: p.id,
            providerName: `${p.name} (${p.capability})`,
            modelId: m.id,
            modelName: m.name,
          });
        }
